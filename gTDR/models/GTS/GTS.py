...
from gTDR.models.GTS.cell import DCGRUCell
import numpy as np
...
class GTS(nn.Module, Seq2SeqAttrs):
    def __init__(self, temperature, **kwargs):
        super().__init__()
        model_kwargs = kwargs.get('model_para')
        ...
        print('Logging via', self._log_dir+'runs/')
    ...
    def _get_log_dir(kwargs):
        ...
        if log_dir is None:
            batch_size = kwargs['data_para'].get('batch_size')
            learning_rate = kwargs['train'].get('base_lr')
            max_diffusion_step = kwargs['model_para'].get('max_diffusion_step')
            num_rnn_layers = kwargs['model_para'].get('num_rnn_layers')
            rnn_units = kwargs['model_para'].get('rnn_units')
            structure = '-'.join(
                ['%d' % rnn_units for _ in range(num_rnn_layers)])
            horizon = kwargs['model_para'].get('horizon')
            filter_type = kwargs['model_para'].get('filter_type')
            filter_type_abbr = 'L'
