...
from gTDR.utils.SALIENT.fast_trainer.shufflers import DistributedShuffler
...
def get_ddp_config(job_dir: str, total_num_nodes: int,
    ...
    print(f"DDP dir is {job_dir}", flush=True)
    while True:
        node_list = os.listdir(job_dir)
        print(f"Node list is {str(node_list)}", flush=True)
        print(
            f"Length is {len(node_list)} and waiting for {total_num_nodes}",
            flush=True,
        )
